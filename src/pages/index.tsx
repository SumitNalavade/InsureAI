...
function App() {
  ...
  const resetMutation = useMutation({
    mutationFn: async () => {
      const response = await fetch('http://127.0.0.1:5328/api/reset', {
        method: 'POST'
      });
      ...
      <div className="flex w-full max-h-full h-full overflow-y-hidden">
        ...
        <div className="w-3/6 bg-gray-100 p-2 flex flex-col">
          <div ref={containerRef} className="flex-grow overflow-y-auto">
            ...
            { mutation.isPending && (<div className='p-2'>
              <span className="loading loading-dots loading-md text-gray-400"></span>
            </div>)}
          </div>
          <form onSubmit={handleQuestionAsked} className="mt-2 flex justify-center input-group">
            <input placeholder="Ask me a question" value={input} onChange={(evt) => setInput(evt.target.value)} className="input max-w-3xl w-full mx-auto h-full min-h-10 bg-gray-200 focus:outline-none rounded-full focus:border-none" />
          </form>
        </div>
      </div>
